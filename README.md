...
<p align="center">
  ...
  <a href="#supported-coding-agents"><img alt="Agents" src="https://img.shields.io/badge/agent-Pi%20SDK%20%2B%20BYOK%20proxy-black?style=flat-square" /></a>
...
**Open Design (OD) is the open-source alternative.** Same loop, same artifact-first mental model, none of the lock-in. The Node.js daemon hosts Pi directly, wires it into a skill-driven design workflow, and can move execution to worker processes for cluster deployments. It runs locally with `pnpm tools-dev`, can deploy the web layer to Vercel, and stays BYOK at every layer.
...
- [**`OpenCoworkAI/open-codesign`**](https://github.com/OpenCoworkAI/open-codesign) — the UX north star and our closest peer. The first open-source Claude-Design alternative. We borrow its streaming-artifact loop, its sandboxed-iframe preview pattern (vendored React 18 + Babel), its live agent panel (todos + tool calls + interruptible generation), and its five-format export list (HTML / PDF / PPTX / ZIP / Markdown). We deliberately diverge on form factor — they are a desktop Electron app bundling [`pi-ai`][piai]; we are a web app + Node.js daemon that hosts Pi SDK directly and can hand runs to workers.
- [**`multica-ai/multica`**](https://github.com/multica-ai/multica) — the daemon-and-runtime architecture. The local daemon as the only privileged process, agent execution as a bounded runtime contract, and the agent-as-teammate worldview.

## At a glance

| | What you get |
|---|---|
| **Coding agent** | Embedded Pi SDK hosted by the Node.js daemon — no local coding-agent CLI discovery, no GUI-inherited PATH surprises, same event stream for text, thinking, tool calls, and tool results |
...
| **Agent runtime** | Local daemon hosts Pi in your project folder — agent gets project-bounded `Read`, `Write`, `Edit`, and `Bash` tools through `ProjectFs`, and Compose can move execution to separate workers |
...
### 1 · Pi is hosted by the daemon.

Open Design no longer asks the user's machine to provide a coding-agent CLI. The daemon embeds Pi through `@earendil-works/pi-coding-agent`, owns model selection, streams Pi thinking/tool events into the UI, and keeps file access behind `ProjectFs`. For cluster deployments the same `AgentRuntime` boundary can enqueue jobs to workers while keeping the browser contract unchanged. BYOK API mode remains available for provider-proxy flows.
...
The daemon creates a project workspace under `.od/projects/<id>/` and gives Pi bounded `Read`, `Write`, `Edit`, and `Bash` tools there. It can read the active skill's side files, inspect generated CSS, write `brand-spec.md`, drop images, and produce `.pptx` / `.zip` / `.pdf` files that show up in the file workspace as download chips when the turn ends. Sessions, conversations, messages, tabs, and run events persist locally; cluster mode can move run history to Postgres and dispatch to Redis-backed workers.
...
| Agent transport | Embedded Pi SDK through `AgentRuntime`; inline for local dev, queued worker mode for Compose/cluster deployments, with Pi thinking/tool events mirrored to the same SSE stream |
...
1. Selects the daemon-hosted Pi SDK agent.
...
│   │   │   ├── agent-runtime.ts   ← inline/worker Pi runtime boundary
...
- **Worker-friendly execution.** Long prompts and tool access now cross a single `AgentRuntime` boundary. Local dev can run Pi inline, while cluster deployments enqueue the same run for an `agent-worker` backed by Redis and Postgres.
...
Open Design ships one daemon-hosted coding agent: Pi SDK. The catalog exposed by
[`apps/daemon/src/agents.ts`](apps/daemon/src/agents.ts) keeps the existing web
contract shape but points at `node:pi-sdk` / `embedded`; it never resolves or
spawns a user PATH binary. Models are populated from Pi's model registry with a
curated fallback list when registry discovery is unavailable.

BYOK proxy mode is still available through
`POST /api/proxy/{provider}/stream` for Anthropic, OpenAI-compatible, Azure
OpenAI, and Gemini providers. It is SSRF-guarded with loopback local providers
allowed, non-loopback internal ranges blocked, and upstream redirects disabled.
...
| [**`multica-ai/multica`**](https://github.com/multica-ai/multica) | The daemon/runtime architecture lineage: local daemon as the privileged process and agent-as-teammate worldview. The current implementation replaces PATH scanning with an embedded Pi SDK runtime. |
...
- [x] Daemon-hosted Pi SDK agent + skill registry + design-system catalog
...
- **Wire up a new agent runtime** — implement the [`AgentRuntime`](apps/daemon/src/agent-runtime.ts) boundary and expose it through [`apps/daemon/src/agents.ts`](apps/daemon/src/agents.ts).
