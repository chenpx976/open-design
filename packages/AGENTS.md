...
- `packages/platform`: generic OS process primitives. Includes stamp serialization, command parsing, process matching/search, and packaged-runtime PATH helpers; it must consume the `sidecar-proto` descriptor and must not hard-code `--od-stamp-*` details. Coding-agent runtime selection no longer uses platform PATH discovery.
